...
# --- GESTION DES PERMISSIONS ANDROID DYNAMIQUES ---
if platform == 'android':
    from android.permissions import request_permissions, check_permission, Permission

# --- IMPORTATION SÉCURISÉE DE ZBARCAM (Empêche les crashs hors Android) ---
try:
...
<PageScannerQR>:
    ...
    MDBoxLayout:
        ...
        MDTopAppBar:
            ...
            left_action_items: [["arrow-left", lambda x: app.fermer_scanner()]]
...
class PageScannerQR(Screen): pass
...
class ZelyphApp(MDApp):
    ...
    def on_start(self):
        """Demande explicite des permissions Android au démarrage de l'application."""
        if platform == 'android':
            permissions_liste = [
                Permission.CAMERA,
            ...
            request_permissions(permissions_liste)
    ...
    # --- Ajouter un ami via son code ou par Caméra ---
    def ouvrir_dialogue_ajouter_code(self):
        ...
        btn_camera = MDFlatButton(
            text="Scanner avec la caméra",
            icon="qrcode-scan",
            on_release=lambda x: self.ouvrir_scanner_camera()
        )
        box.add_widget(self.input_code_ami)
        box.add_widget(btn_camera)
    ...
    def ouvrir_scanner_camera(self):
        ...
        if platform == 'android':
            if not check_permission(Permission.CAMERA):
                ...
                self.afficher_message_info("Autorisation requise", "Veuillez accepter l'accès à l'appareil photo.")
                return

        if ZBarCam is None:
            self.afficher_message_info("Erreur", "La bibliothèque ZBarCam n'est pas installée sur ce périphérique.")
            return

        screen_scanner = self.root.get_screen('scanner')
        conteneur = screen_scanner.ids.conteneur_cam
        conteneur.clear_widgets()

        self.zbarcam = ZBarCam()
        self.zbarcam.bind(symbols=self.sur_detection_qr)
        conteneur.add_widget(self.zbarcam)

        self.root.current = 'scanner'

    def sur_detection_qr(self, instance, symbols):
        if symbols:
            for symbol in symbols:
                code_scanne = symbol.data.decode('utf-8').strip().upper()
                if code_scanne:
                    self.fermer_scanner()
                    self.rechercher_utilisateur_par_code(code_scanne)
                    break

    def fermer_scanner(self):
        if self.zbarcam:
            self.zbarcam.stop()
        self.root.current = 'accueil'
    ...
    # --- Afficher et Accepter les demandes reçues ---
    ...
    def sauvegarder_modification_profil(self):
        ...
        nouvel_avatar = self.input_edit_avatar.text.strip() or "logo.png"
